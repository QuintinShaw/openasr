...
/// Validates the auxiliary package through the same `PackVerifier` ingress as
/// ASR and every other auxiliary route. Hy-MT2 no longer owns a public
/// path-based probe/cache that could drift from the package contract.
fn validate_hymt2_translation_pack(path: &Path) -> Result<(), String> {
    openasr_core::verify_native_runtime_model_pack_path(path).map_err(|error| {
        format!(
            "{}: {}",
            RealtimeTranslationCapability::REASON_MODEL_UNSUPPORTED,
            error
