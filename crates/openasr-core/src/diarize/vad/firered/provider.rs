//! [`LongFormVadProvider`] backed by the neural FireRedVAD DFSMN model.
//!
//! Same shape as `crate::diarize::vad::provider::SileroVadProvider`: the
//! model emits one speech probability per 10 ms frame; this layer turns that
//! sequence into speech spans using the same threshold / min-speech /
//! min-silence hysteresis the other long-form VAD providers use, so the
//! `Auto` planner can weigh it against Energy/Fixed on equal footing.

use thiserror::Error;

use super::frontend::SAMPLE_RATE_HZ;
use super::model::{FRAME_SHIFT_MS, FireRedVadModel};
use super::weights::FireRedVadWeightsError;
use crate::longform::{
    LongFormOptions, LongFormVadProvider, LongFormVadProviderKind, LongFormVadSlice,
};

#[derive(Debug, Error)]
pub enum FireRedVadError {
    #[error("firered VAD model is unavailable: {0}")]
    Unavailable(#[from] FireRedVadWeightsError),
}

/// Neural VAD provider over the process-wide shared FireRedVAD model. Cheap
/// to construct (it only borrows the model), so build one per request.
pub struct FireRedVadProvider {
    model: &'static FireRedVadModel,
}

impl FireRedVadProvider {
    /// Borrow the shared FireRedVAD model. Returns `None` when the vendored
...
impl LongFormVadProvider for FireRedVadProvider {
    ...
    fn compute_speech_slices(
        ...
        if sample_rate_hz != SAMPLE_RATE_HZ {
            return Err(format!(
                "firered VAD requires {SAMPLE_RATE_HZ} Hz mono audio, got {sample_rate_hz} Hz"
            ));
        }
        if samples.is_empty() {
            return Ok(Vec::new());
...
/// Convert per-frame speech probabilities into sample-space speech spans
/// with threshold gating plus min-speech / min-silence hysteresis.
fn spans_from_probs(
    probs: &[f32],
    total_samples: usize,
    options: &LongFormOptions,
