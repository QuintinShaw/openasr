...
impl Qwen3AsrLlmWholeDecoderGraphExecutor {
    ...
    fn llm_prefill_uses_flash_attention(&self, token_count: usize, kv_span: usize) -> bool {
        ...
        !matches!(self.runner.backend_kind(), GgmlCpuGraphBackend::Gpu)
    }
...
mod tests {
    ...
    fn qwen_llm_prefill_flash_policy_is_fail_closed_on_discrete_gpu() {
        // Pure policy unit: mirrors llm_prefill_uses_flash_attention without
        // standing up a live backend. Wide multi-query + long KV must never
        // select flash on the discrete-GPU lane.
        assert!(qwen_llm_prefill_flash_attention_for_test(
            GgmlCpuGraphBackend::Gpu,
            /*token_count=*/ 1,
            /*kv_span=*/ 128
        ));
    ...
    fn qwen_llm_prefill_flash_attention_for_test(
