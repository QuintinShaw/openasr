...
mod tests {
    ...
    #[test]
    #[ignore = "host-local: compares X-ASR layer0 attention weights reference with exported ONNX debug tensors"]
    fn layer0_attention_weights_reference_matches_onnx_debug_when_present() {
        ...
        let expected = read_f32_file(&softmax_path);
        ...
        let key_padding_mask = streaming_key_padding_mask(metadata.left_context_len[0], 24, 61)
            .expect("key padding mask");
        let output = self_attention_weights_streaming_reference(
            attn,
            &input,
            24,
            192,
            metadata.num_heads[0],
            metadata.query_head_dims[0],
            metadata.left_context_len[0],
            None,
            Some(&key_padding_mask),
        )
        .expect("attention weights");

        assert_eq!(output.weights.len(), expected.len());
        let diff = max_abs_diff(&output.weights, &expected);
        assert!(
            diff <= 2.0e-2,
            "layer0 attention weights max abs diff {diff} exceeds tolerance"
        );
    }

    #[test]
    #[ignore = "host-local: compares X-ASR layer0 attention value projections with exported ONNX debug tensors"]
    fn layer0_attention_value_references_match_onnx_debug_when_present() {
        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tmp/xasr-test/out");
        let add6_path = root.join("oracle-layer0-ff-debug-480ms.add6.f32");
        let softmax_path = root.join("oracle-layer0-debug-480ms.softmax.f32");
        let nonlin_path = root.join("oracle-layer0-attn-values-debug-480ms.nonlin.f32");
        let add8_path = root.join("oracle-layer0-attn-values-debug-480ms.add8.f32");
        let self1_path = root.join("oracle-layer0-attn-values-debug-480ms.self1.f32");
        let add10_path = root.join("oracle-layer0-attn-values-debug-480ms.add10.f32");
        let pack = root.join("xasr-zh-en-onnx-fp16.oasr");
        if !add6_path.exists()
            || !softmax_path.exists()
            || !nonlin_path.exists()
        ...
        {
            eprintln!(
                "skipping: missing attention value oracle files or {}",
                pack.display()
            );
            return;
        }

        let add6 = read_f32_file(&add6_path);
        let softmax = read_f32_file(&softmax_path);
        let expected_nonlin = read_f32_file(&nonlin_path);
        let expected_add8 = read_f32_file(&add8_path);
        let expected_self1 = read_f32_file(&self1_path);
        let expected_add10 = read_f32_file(&add10_path);
        let reader = GgufTensorDataReader::from_path(&pack).expect("reader");
        let metadata = read_gguf_metadata(&pack).expect("metadata");
        let metadata = parse_xasr_zipformer_execution_metadata(&metadata).expect("metadata parse");
        let weights = load_xasr_encoder_weights(&reader, &metadata).expect("weights");
        let layer = &weights.stacks[0].layers[0];
        let k_len = metadata.left_context_len[0] + 24;
        let selected_attn = &softmax[..24 * k_len];
        ...
        .expect("nonlin attention");
        let add8: Vec<f32> = add6
            .iter()
            .zip(nonlin.rows.iter())
            .map(|(&lhs, &rhs)| lhs + rhs)
            .collect();
        let self1 = self_attention_streaming_reference(
            &layer.self_attn1,
        ...
        .expect("self attention1");
        let add10: Vec<f32> = add8
            .iter()
            .zip(self1.rows.iter())
            .map(|(&lhs, &rhs)| lhs + rhs)
            .collect();

        let nonlin_diff = max_abs_diff(&nonlin.rows, &expected_nonlin);
        let add8_diff = max_abs_diff(&add8, &expected_add8);
        let self1_diff = max_abs_diff(&self1.rows, &expected_self1);
        let add10_diff = max_abs_diff(&add10, &expected_add10);
        assert!(
            nonlin_diff <= 2.0e-2,
            "layer0 nonlin attention max abs diff {nonlin_diff} exceeds tolerance"
        );
        assert!(
            add8_diff <= 2.0e-2,
            "layer0 nonlin residual max abs diff {add8_diff} exceeds tolerance"
        );
        assert!(
            self1_diff <= 2.0e-2,
            "layer0 self attention1 max abs diff {self1_diff} exceeds tolerance"
        );
        assert!(
            add10_diff <= 2.0e-2,
            "layer0 self attention1 residual max abs diff {add10_diff} exceeds tolerance"
        );
    }

    #[test]
    #[ignore = "host-local: compares X-ASR layer0 conv module reference with exported ONNX debug tensors"]
    fn layer0_conv_module1_reference_matches_onnx_debug_when_present() {
        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tmp/xasr-test/out");
        let input_path = root.join("oracle-layer0-conv1-debug-480ms.add10.f32");
        let depthwise_path = root.join("oracle-layer0-conv1-debug-480ms.add14.f32");
        let conv_path = root.join("oracle-layer0-conv1-debug-480ms.conv1.f32");
        let add_path = root.join("oracle-layer0-conv1-debug-480ms.add15.f32");
        let pack = root.join("xasr-zh-en-onnx-fp16.oasr");
        if !input_path.exists()
            || !depthwise_path.exists()
            || !conv_path.exists()
            || !add_path.exists()
            || !pack.exists()
        {
        ...
        let input = read_f32_file(&input_path);
        let expected_depthwise = read_f32_file(&depthwise_path);
        let expected_conv = read_f32_file(&conv_path);
        let expected_add = read_f32_file(&add_path);
        let reader = GgufTensorDataReader::from_path(&pack).expect("reader");
        let metadata = read_gguf_metadata(&pack).expect("metadata");
        let metadata = parse_xasr_zipformer_execution_metadata(&metadata).expect("metadata parse");
        let weights = load_xasr_encoder_weights(&reader, &metadata).expect("weights");
        let conv = &weights.stacks[0].layers[0].conv_module1;
        let output = convolution_module_streaming_reference(conv, &input, 24, 192, None, None)
            .expect("conv");

        assert_eq!(output.depthwise_rows.len(), expected_depthwise.len());
        assert_eq!(output.rows.len(), expected_conv.len());
        let depthwise_diff = max_abs_diff(&output.depthwise_rows, &expected_depthwise);
        let conv_diff = max_abs_diff(&output.rows, &expected_conv);
        let residual_add: Vec<f32> = input
            .iter()
            .zip(output.rows.iter())
            .map(|(&lhs, &rhs)| lhs + rhs)
            .collect();
        let add_diff = max_abs_diff(&residual_add, &expected_add);

        assert!(
            depthwise_diff <= 2.0e-2,
            "layer0 conv_module1 depthwise max abs diff {depthwise_diff} exceeds tolerance"
        );
        assert!(
            conv_diff <= 2.0e-2,
            "layer0 conv_module1 output max abs diff {conv_diff} exceeds tolerance"
        );
        assert!(
            add_diff <= 2.0e-2,
            "layer0 conv_module1 residual add max abs diff {add_diff} exceeds tolerance"
        );
    }

    #[test]
    #[ignore = "host-local: compares X-ASR layer0 feed-forward reference with exported ONNX debug tensors"]
    fn layer0_feed_forward_reference_matches_onnx_debug_when_present() {
        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tmp/xasr-test/out");
        let input_path = root.join("oracle-layer0-ff-debug-480ms.input.f32");
        let ff1_path = root.join("oracle-layer0-ff-debug-480ms.ff1.f32");
        let add6_path = root.join("oracle-layer0-ff-debug-480ms.add6.f32");
        let add15_path = root.join("oracle-layer0-conv1-debug-480ms.add15.f32");
        let ff2_path = root.join("oracle-layer0-ff-debug-480ms.ff2.f32");
        ...
        {
            eprintln!(
                "skipping: missing feed-forward oracle files or {}",
                pack.display()
            );
            return;
        }
        ...
        let input = read_f32_file(&input_path);
        let expected_ff1 = read_f32_file(&ff1_path);
        let expected_add6 = read_f32_file(&add6_path);
        let add15 = read_f32_file(&add15_path);
        let expected_ff2 = read_f32_file(&ff2_path);
        let expected_add16 = read_f32_file(&add16_path);
        let reader = GgufTensorDataReader::from_path(&pack).expect("reader");
        let metadata = read_gguf_metadata(&pack).expect("metadata");
        let metadata = parse_xasr_zipformer_execution_metadata(&metadata).expect("metadata parse");
        let weights = load_xasr_encoder_weights(&reader, &metadata).expect("weights");
        ...
        let ff1 = feed_forward_reference(&layer.feed_forward1, &input, 24, 192).expect("ff1");
        let add6: Vec<f32> = input
            .iter()
            .zip(ff1.iter())
            .map(|(&lhs, &rhs)| lhs + rhs)
            .collect();
        let ff2 = feed_forward_reference(&layer.feed_forward2, &add15, 24, 192).expect("ff2");
        let add16: Vec<f32> = add15
            .iter()
            .zip(ff2.iter())
            .map(|(&lhs, &rhs)| lhs + rhs)
            .collect();

        let ff1_diff = max_abs_diff(&ff1, &expected_ff1);
        let add6_diff = max_abs_diff(&add6, &expected_add6);
        let ff2_diff = max_abs_diff(&ff2, &expected_ff2);
        let add16_diff = max_abs_diff(&add16, &expected_add16);
        assert!(
            ff1_diff <= 2.0e-2,
            "layer0 feed_forward1 max abs diff {ff1_diff} exceeds tolerance"
        );
        assert!(
            add6_diff <= 2.0e-2,
            "layer0 feed_forward1 residual max abs diff {add6_diff} exceeds tolerance"
        );
        assert!(
            ff2_diff <= 2.0e-2,
            "layer0 feed_forward2 max abs diff {ff2_diff} exceeds tolerance"
        );
        assert!(
            add16_diff <= 2.0e-2,
            "layer0 feed_forward2 residual max abs diff {add16_diff} exceeds tolerance"
        );
    }

    #[test]
    #[ignore = "host-local: compares a full X-ASR layer0 Zipformer reference pass with exported ONNX debug tensors"]
    fn layer0_full_reference_matches_onnx_debug_when_present() {
        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tmp/xasr-test/out");
        let input_path = root.join("oracle-layer0-ff-debug-480ms.input.f32");
        let softmax_path = root.join("oracle-layer0-debug-480ms.softmax.f32");
        let bypass_mid_path = root.join("oracle-layer0-second-half-debug-480ms.bypass_mid.f32");
        let self2_path = root.join("oracle-layer0-second-half-debug-480ms.self2.f32");
        let add18_path = root.join("oracle-layer0-second-half-debug-480ms.add18.f32");
        let conv2_depthwise_path =
            root.join("oracle-layer0-second-half-debug-480ms.conv2_depthwise.f32");
        let conv2_path = root.join("oracle-layer0-second-half-debug-480ms.conv2.f32");
        let add23_path = root.join("oracle-layer0-second-half-debug-480ms.add23.f32");
        let ff3_path = root.join("oracle-layer0-second-half-debug-480ms.ff3.f32");
        let add24_path = root.join("oracle-layer0-second-half-debug-480ms.add24.f32");
        let norm_path = root.join("oracle-layer0-second-half-debug-480ms.norm.f32");
        let bypass_path = root.join("oracle-layer0-second-half-debug-480ms.bypass.f32");
        let pack = root.join("xasr-zh-en-onnx-fp16.oasr");
        let required = [
            &input_path,
            &softmax_path,
        ...
        if required.iter().any(|path| !path.exists()) {
            eprintln!("skipping: missing layer0 full reference oracle files");
            return;
        }
        ...
        let input = read_f32_file(&input_path);
        let softmax = read_f32_file(&softmax_path);
        let expected_bypass_mid = read_f32_file(&bypass_mid_path);
        let expected_self2 = read_f32_file(&self2_path);
        let expected_add18 = read_f32_file(&add18_path);
        let expected_conv2_depthwise = read_f32_file(&conv2_depthwise_path);
        let expected_conv2 = read_f32_file(&conv2_path);
        let expected_add23 = read_f32_file(&add23_path);
        let expected_ff3 = read_f32_file(&ff3_path);
        let expected_add24 = read_f32_file(&add24_path);
        let expected_norm = read_f32_file(&norm_path);
        let expected_bypass = read_f32_file(&bypass_path);
        let reader = GgufTensorDataReader::from_path(&pack).expect("reader");
        let metadata = read_gguf_metadata(&pack).expect("metadata");
        let metadata = parse_xasr_zipformer_execution_metadata(&metadata).expect("metadata parse");
        let weights = load_xasr_encoder_weights(&reader, &metadata).expect("weights");
        ...
        let k_len = left_context_len + FRAMES;
        let selected_attn = &softmax[..FRAMES * k_len];

        let ff1 = feed_forward_reference(&layer.feed_forward1, &input, FRAMES, DIM).expect("ff1");
        let add6 = add_same_shape(&input, &ff1);
        let nonlin = nonlin_attention_streaming_reference(
            &layer.nonlin_attention,
            &add6,
            selected_attn,
        ...
        .expect("nonlin attention");
        let add8 = add_same_shape(&add6, &nonlin.rows);
        let self1 = self_attention_streaming_reference(
            &layer.self_attn1,
            &add8,
            &softmax,
        ...
        .expect("self attention1");
        let add10 = add_same_shape(&add8, &self1.rows);
        let conv1 = convolution_module_streaming_reference(
            &layer.conv_module1,
            &add10,
            FRAMES,
        ...
        .expect("conv1");
        let add15 = add_same_shape(&add10, &conv1.rows);
        let ff2 = feed_forward_reference(&layer.feed_forward2, &add15, FRAMES, DIM).expect("ff2");
        let add16 = add_same_shape(&add15, &ff2);
        let bypass_mid = bypass_reference(&input, &add16, &layer.bypass_mid_scale, FRAMES, DIM)
            .expect("bypass_mid");
        ...
        .expect("self attention2");
        let add18 = add_same_shape(&bypass_mid, &self2.rows);
        let conv2 = convolution_module_streaming_reference(
            &layer.conv_module2,
            &add18,
            FRAMES,
        ...
        .expect("conv2");
        let add23 = add_same_shape(&add18, &conv2.rows);
        let ff3 = feed_forward_reference(&layer.feed_forward3, &add23, FRAMES, DIM).expect("ff3");
        let add24 = add_same_shape(&add23, &ff3);
        let mut norm = add24.clone();
        bias_norm_last_dim(&mut norm, DIM, &layer.norm_bias, layer.norm_log_scale[0])
            .expect("norm");
        let bypass =
            bypass_reference(&input, &norm, &layer.bypass_scale, FRAMES, DIM).expect("bypass");
        let layer_output = zipformer_layer_streaming_reference(
            layer,
            &input,
            FRAMES,
            DIM,
        ...
        assert_max_abs_diff(
            "layer0 self attention2",
            &self2.rows,
            &expected_self2,
            TOLERANCE,
        );
        assert_max_abs_diff(
            "layer0 self attention2 residual",
            &add18,
            &expected_add18,
            TOLERANCE,
        );
        assert_max_abs_diff(
            "layer0 conv_module2 depthwise",
            &conv2.depthwise_rows,
            &expected_conv2_depthwise,
            TOLERANCE,
        );
        assert_max_abs_diff(
            "layer0 conv_module2",
        ...
        assert_max_abs_diff(
            "layer0 conv_module2 residual",
            &add23,
            &expected_add23,
            TOLERANCE,
        );
        assert_max_abs_diff("layer0 feed_forward3", &ff3, &expected_ff3, TOLERANCE);
        assert_max_abs_diff(
            "layer0 feed_forward3 residual",
            &add24,
            &expected_add24,
            TOLERANCE,
        );
        assert_max_abs_diff("layer0 norm", &norm, &expected_norm, TOLERANCE);
        assert_max_abs_diff("layer0 bypass", &bypass, &expected_bypass, TOLERANCE);
        assert_max_abs_diff(
            "layer0 reusable layer reference",
            &layer_output.rows,
            &expected_bypass,
            TOLERANCE,
        );
    ...
    #[test]
    #[ignore = "host-local: compares a two-layer X-ASR stack0 reference pass with exported ONNX debug tensors"]
    fn stack0_two_layer_reference_matches_onnx_debug_when_present() {
        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tmp/xasr-test/out");
        let input_path = root.join("oracle-layer0-ff-debug-480ms.input.f32");
        let layer0_path = root.join("oracle-stack0-debug-480ms.layer0.f32");
        let layer1_path = root.join("oracle-stack0-debug-480ms.layer1.f32");
        let pack = root.join("xasr-zh-en-onnx-fp16.oasr");
        if !input_path.exists() || !layer0_path.exists() || !layer1_path.exists() || !pack.exists()
        {
            eprintln!("skipping: missing stack0 reference oracle files");
            return;
        }
        ...
        let input = read_f32_file(&input_path);
        let expected_layer0 = read_f32_file(&layer0_path);
        let expected_layer1 = read_f32_file(&layer1_path);
        let reader = GgufTensorDataReader::from_path(&pack).expect("reader");
        let metadata = read_gguf_metadata(&pack).expect("metadata");
        let metadata = parse_xasr_zipformer_execution_metadata(&metadata).expect("metadata parse");
        let weights = load_xasr_encoder_weights(&reader, &metadata).expect("weights");
        let stack = &weights.stacks[0];

        let layer0 = zipformer_layer_streaming_reference(
            &stack.layers[0],
            &input,
            FRAMES,
            DIM,
        ...
        .expect("layer0");
        let layer1 = zipformer_layer_streaming_reference(
            &stack.layers[1],
            &layer0.rows,
            FRAMES,
            DIM,
    ...
    #[test]
    #[ignore = "host-local: compares a two-layer X-ASR stack1 reference pass with exported ONNX debug tensors"]
    fn stack1_two_layer_reference_matches_onnx_debug_when_present() {
        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tmp/xasr-test/out");
        let input_path = root.join("oracle-downsample-stack1-debug-480ms.output.f32");
        let layer0_path = root.join("oracle-stack1-debug-480ms.layer0.f32");
        let layer1_path = root.join("oracle-stack1-debug-480ms.layer1.f32");
        let pack = root.join("xasr-zh-en-onnx-fp16.oasr");
        if !input_path.exists() || !layer0_path.exists() || !layer1_path.exists() || !pack.exists()
        {
            eprintln!("skipping: missing stack1 reference oracle files");
            return;
        }

        const FRAMES: usize = 12;
        const DIM: usize = 256;
        const VALID_LEFT_CONTEXT: usize = 31;
        const TOLERANCE: f32 = 2.0e-2;
        ...
        let layer0 = zipformer_layer_streaming_reference(
            ...
            metadata.query_head_dims[1],
            metadata.left_context_len[1],
            VALID_LEFT_CONTEXT,
            XasrZipformerLayerReferenceCaches::default(),
        )
        .expect("stack1 layer0");
        let layer1 = zipformer_layer_streaming_reference(
            &stack.layers[1],
            &layer0.rows,
            FRAMES,
    ...
    #[test]
    #[ignore = "host-local: compares a four-layer X-ASR stack2 reference pass with exported ONNX debug tensors"]
    fn stack2_four_layer_reference_matches_onnx_debug_when_present() {
        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tmp/xasr-test/out");
        let input_path = root.join("oracle-stack1-combine-debug-480ms.padded.f32");
        let downsample_path = root.join("oracle-stack2-debug-480ms.downsample.f32");
        let layer_paths = [
            root.join("oracle-stack2-debug-480ms.layer0.f32"),
            root.join("oracle-stack2-debug-480ms.layer1.f32"),
            root.join("oracle-stack2-debug-480ms.layer2.f32"),
            root.join("oracle-stack2-debug-480ms.layer3.f32"),
        ];
        let pack = root.join("xasr-zh-en-onnx-fp16.oasr");
        if !input_path.exists()
            || !downsample_path.exists()
            || layer_paths.iter().any(|path| !path.exists())
            || !pack.exists()
        {
            eprintln!("skipping: missing stack2 reference oracle files");
            return;
        }

        const INPUT_FRAMES: usize = 24;
        const INPUT_DIM: usize = 512;
        const FRAMES: usize = 6;
        const DIM: usize = 512;
        const VALID_LEFT_CONTEXT: usize = 16;
        const TOLERANCE: f32 = 2.0e-2;

        let input = read_f32_file(&input_path);
        let expected_downsample = read_f32_file(&downsample_path);
        let expected_layers = layer_paths
            .iter()
            .map(|path| read_f32_file(path))
            .collect::<Vec<_>>();
        let reader = GgufTensorDataReader::from_path(&pack).expect("reader");
        let metadata = read_gguf_metadata(&pack).expect("metadata");
        let metadata = parse_xasr_zipformer_execution_metadata(&metadata).expect("metadata parse");
        let weights = load_xasr_encoder_weights(&reader, &metadata).expect("weights");
        let stack = &weights.stacks[2];
        let downsample = downsample_streaming_reference(
            &input,
            INPUT_FRAMES,
            INPUT_DIM,
            DIM,
            stack.downsample_bias.as_ref().expect("stack2 downsample"),
        )
        .expect("stack2 downsample");
        assert_eq!(downsample.frames, FRAMES);
        assert_eq!(downsample.dim, DIM);
        assert_max_abs_diff(
            "stack2 downsample output",
            &downsample.rows,
            &expected_downsample,
            TOLERANCE,
        );

        let mut rows = downsample.rows;
        for (layer_index, layer) in stack.layers.iter().enumerate() {
            let output = zipformer_layer_streaming_reference(
                ...
                metadata.query_head_dims[2],
                metadata.left_context_len[2],
                VALID_LEFT_CONTEXT,
                XasrZipformerLayerReferenceCaches::default(),
            )
            .unwrap_or_else(|error| panic!("stack2 layer{layer_index}: {error}"));
            assert_max_abs_diff(
                &format!("stack2 layer{layer_index} final"),
                &output.rows,
    ...
    #[test]
    #[ignore = "host-local: compares X-ASR embed reference with exported ONNX debug tensors"]
    fn encoder_embed_reference_matches_onnx_debug_when_present() {
        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tmp/xasr-test/out");
        let x_path = root.join("oracle-encoder-debug-480ms.x.f32");
        let out_path = root.join("oracle-encoder-debug-480ms.out_norm.f32");
        let pack = root.join("xasr-zh-en-onnx-fp16.oasr");
        if !x_path.exists() || !out_path.exists() || !pack.exists() {
            eprintln!(
                "skipping: missing {}, {}, or {}",
                x_path.display(),
                out_path.display(),
        ...
        let x = read_f32_file(&x_path);
        let expected = read_f32_file(&out_path);
        let reader = GgufTensorDataReader::from_path(&pack).expect("reader");
        let metadata = read_gguf_metadata(&pack).expect("metadata");
        let metadata = parse_xasr_zipformer_execution_metadata(&metadata).expect("metadata parse");
        let weights = load_xasr_encoder_weights(&reader, &metadata).expect("weights");
        let output = encode_embed_reference(&weights.embed, &x, 61, 80, None).expect("embed");

        assert_eq!(output.frames, 24);
        assert_eq!(output.dim, 192);
        assert_eq!(output.rows.len(), expected.len());
        let mut max_abs = 0.0_f32;
        for (&got, &want) in output.rows.iter().zip(expected.iter()) {
            max_abs = max_abs.max((got - want).abs());
        }
        assert!(
            max_abs <= 6.0e-3,
            "encoder embed max abs diff {max_abs} exceeds tolerance"
        );
    }
