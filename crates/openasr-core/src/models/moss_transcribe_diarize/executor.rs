...
impl Seq2SeqGreedyDecodeStepExecutor for MossTdGreedyStepExecutor<'_> {
    fn decode_step_logits(
        ...
        if let Some(prompt_embeddings) = self.prompt_embeddings.take() {
            ...
            if std::env::var_os("OPENASR_MOSS_TD_OOM_TRACE").is_some() {
                eprintln!(
                    "moss-oom-trace decode step: entering resident prefill prompt_tokens={}",
                    prompt_embeddings.token_count
                );
            }
            let prefill = self
                .decoder
            ...
            if std::env::var_os("OPENASR_MOSS_TD_OOM_TRACE").is_some() {
                eprintln!("moss-oom-trace decode step: resident prefill completed");
            }
...
fn run_moss_td_decoder_with_cached_runtime(
    ...
    with_thread_local_cached_mut_by_key(
        ...
        |decoder| {
            ...
            if std::env::var_os("OPENASR_MOSS_TD_OOM_TRACE").is_some() {
                eprintln!(
                    "moss-oom-trace decoder prefill: prompt_tokens={} kv_capacity={} generated_budget={}",
                    prompt_embeddings.token_count, request_kv_cache_positions, max_generated_tokens
                );
            }
...
mod tests {
    ...
    fn accelerated_aishell4_three_minute_smoke_completes_with_structured_transcript() {
        ...
        assert!(
            ...
            "accelerated AISHELL-4 transcript must retain parseable speaker/time structure"
        );
    }
}
