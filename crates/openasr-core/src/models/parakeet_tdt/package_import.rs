...
pub fn convert_local_parakeet_tdt_source_to_runtime_pack(
    ...
    let vocab_tokens = load_vocab_tokens(&request.source_root, config.vocab_size, "parakeet-tdt")?;
    ...
    let verified = OasrPackWriter::write(
        &request.output_root,
        PackEnvelope::asr(PARAKEET_TDT_GGML_ARCHITECTURE_ID),
        metadata,
        &tensors,
    )
    .map_err(|error| {
        validate_error(format!(
            "parakeet-tdt OASR writer failed for '{}': {error}",
