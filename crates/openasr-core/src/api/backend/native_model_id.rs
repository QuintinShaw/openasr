...
use crate::{GgmlRuntimeSource, parse_model_ref, read_gguf_metadata_from_runtime_source};
...
pub(crate) fn resolve_native_runtime_model_identity_from_source(
    ...
    let (metadata, metadata_read_error) =
        match read_gguf_metadata_from_runtime_source(runtime_source) {
            Ok(metadata) => {
                let metadata = METADATA_MODEL_ID_CANDIDATE_KEYS
                    .iter()
                    .filter_map(|key| {
                        metadata
                            .get_string(key)
    ...
    resolve_native_runtime_model_identity_from_string_metadata(
        &metadata,
        runtime_source.path(),
        explicit_model_id_fallback,
    )
    .map_err(|error| match (error, metadata_read_error) {
        (
            NativeRuntimeModelIdentityError::MissingModelId {
                path,
                mut metadata_error,
            },
            Some(read_error),
        ) => {
            metadata_error.insert_str(0, &format!(" metadata_read_error={read_error};"));
            NativeRuntimeModelIdentityError::MissingModelId {
                path,
                metadata_error,
            }
...
mod tests {
    ...
    fn resolve_local_native_runtime_model_identity(
        runtime_path: &Path,
        explicit_model_id_fallback: Option<&str>,
    ) -> Result<NativeRuntimeModelIdentity, NativeRuntimeModelIdentityError> {
        let runtime_source =
        ...
        resolve_native_runtime_model_identity_from_source(
            &runtime_source,
            explicit_model_id_fallback,
        )
    }
