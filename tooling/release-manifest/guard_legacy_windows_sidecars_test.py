...
class LegacyWindowsSidecarGuardTest(unittest.TestCase):
    def setUp(self) -> None:
        self.policy = {
            "schema_version": 1,
            "legacy_static_sidecar_last_core_version": "0.1.34",
        }

    def test_allows_only_the_frozen_migration_window(self) -> None:
        self.assertTrue(legacy_sidecar_allowed("0.1.32", self.policy))
        self.assertTrue(legacy_sidecar_allowed("v0.1.34", self.policy))
        self.assertFalse(legacy_sidecar_allowed("0.1.35", self.policy))
