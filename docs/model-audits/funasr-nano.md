...
| Resident pool reuse across requests (weights stay resident) | Supported | The shared NativeExecutionServices module owns prepared-runtime pooling and content-id eviction for every admitted family; `resident_runtime_audit` scans the FunASR module for the owner-bound cache primitive. Encoder + adapter + decoder remain resident across requests; idle unload is owned by the service root. Test `cached_encoder_adapter_matches_fresh_build_bit_for_bit`. |
