...
| Resident pool reuse across requests (weights stay resident) | Supported | K2 is derived from the FunASR architecture descriptor's `module_slug = "funasr_nano"`, `PreparedRuntimePool`, NativeExecutionServices ownership, and content-id eviction facets; `resident_runtime_audit` then scans the derived module for the shared owner-bound cache primitive. Encoder + adapter + decoder remain resident across requests; idle unload is owned by the service root. Test `cached_encoder_adapter_matches_fresh_build_bit_for_bit`. |
...
| Model ref alias forms resolve identically everywhere | Supported | Registry resolution green (shared-driver registry test); canonical `openasr.model.architecture=funasr-nano-sanm-adapter-qwen3` on packs. Public catalog row ships canonical quant tags `fp16` / `q8_0` / `q4_k` with aliases `funasr` / `fun-asr-nano`; catalog-wide alias matrix walks every public row after the public flip. Shared `quant_tag_cases.json` covers the quant-tag surface. |
| Golden coverage includes long audio AND a cross-backend parity fixture | Supported | Cross-backend parity fixture present (Metal vs CPU bit-identical). Long audio: ConservativeSeq2SeqV1 ~15s slicing + validated indirectly by the ~40-min frozen WER set (no systematic repetition/drift); bring-up goldens cover short clips. Dedicated multi-minute committed golden fixture is optional polish, not a correctness gap for the public path. |
...
| Registry / catalog / docs wired (MODEL_ONBOARDING checklist done) | Supported | Registry fully wired (arch ids, 5 component descriptors, all registries, native dispatch, pack_quant_audit; registry-resolution test green). Public catalog row + registry card + model card prose (`tooling/publish-model/cards/funasr-nano.toml`) + HF `OpenASR/funasr-nano` packs (fp16/q8_0/q4_k) + ExternalTooling provenance (`funasr_nano_pt_to_safetensors.py`) + signed catalog epoch land on the public flip. |
