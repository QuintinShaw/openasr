...
| Peak RSS/VRAM per shipped quant measured (quiet host) and reconciled against the weights+KV+activations budget; unexplained excess blocks release; catalog RAM requirement matches the measured peak | Deferred | Measured at `08841d4` (quiet-window matrix, 3 runs/cell, raw data `tmp/perf-fr2-mem/`): Metal jfk RSS cold 5717 / warm 7278 MiB, phys_footprint ~3.2-3.5 GiB; Metal long(5min) footprint +0-170MB vs jfk -- reconciles with the ~100MB sliced-KV budget, PASS. CPU long footprint: re-measured at `dae714d` -- **bounded**: 2min (3 slices) 8.877 GiB vs 5min (8 slices) 8.853 GiB are flat, so the +3.39 GiB over jfk is a one-time step at the single-slice(11s)->full-40s-slice transition, not growth with duration. #172 (per-token buffer pool, O(log) allocs) merged but did not move the peak -- the original fragmentation attribution was wrong; current hypothesis (verification in flight): the CPU 40s-slice encoder/prefill compute buffer (naive-attention O(n^2) class) is itself ~3.4 GiB, allocated at the first full slice and reused. Catalog perf badly stale in BOTH fields: measured rtf_cpu 0.905 / rtf_metal 0.535 (warm medians) vs catalog 1.978 / 0.929. Unlock: close the step attribution (then either justify the 40s compute buffer as budgeted or open its optimization), pick the peak_rss_bytes convention (footprint vs maxrss, CPU vs Metal), and sync catalog in one pass. |
