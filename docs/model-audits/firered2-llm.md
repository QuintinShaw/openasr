...
| --- | --- | --- |
| Graph reuse / persistent session (no per-request graph rebuild) | Supported | `run_step_auto`/`run_prefill_auto_last_hidden` on the shared qwen executor (`llm_transformer.rs:1938,1986` as of `dae714d`; merged `4d7d8f8`). Clean medians at merge time (quiet host): Metal RTF 0.760->0.737 (jfk), 0.772->0.732 (zh); direction re-confirmed by the dae714d re-measure (rtf_metal 0.535 warm). Transcripts byte-identical to golden. |
| Op fusion opportunities reviewed (norm+matmul, QKV, rope, ...) | Supported | Shared qwen graph: fused QKV projection load (`llm_transformer.rs:693-710` as of `dae714d`), fused logits head (`:1237-1247`). No firered-specific op stitching added. |
...
| Item | Status | Justification / evidence (+ unlock condition if not Supported) |
| --- | --- | --- |
| KV cache quantization | Supported | Shared phase-1 `q8_0` KV landed in PR #217; production wire-up in PR #234 selects `LlmKvCachePolicy::Q8_0` on CPU/Metal when native-GQA + flash geometry allow and `head_dim` is q8_0-block-aligned (`resolve_production_llm_kv_cache_policy` at whole-decoder construction -- firered2 uses the same qwen-family constructor). Discrete GPU stays host F32 / resident F16 with typed reject. Opt-out: `OPENASR_QWEN_KV_CACHE_F32=1` for golden/parity. Not a catalog quant tag; re-measure peak RSS after default-on before rewriting catalog RAM fields. |
...
| Resident pool reuse across requests (weights stay resident) | Supported | PR #168 (merged `2bc28f5`) established the measured ~1.8-2.0s per-request rebuild win and byte-identical consecutive-call golden. The live implementation has since moved from the historical qwen-family TLS/path cache to a finite executor-owned `AdmittedPinnedRuntimeActorCheckoutPool`: each thread-affine decoder runtime stays on its dedicated owner thread and is keyed by `(PackContentKey, ExecutionLaneKey)`. `PackContentKey` comes from the already-open source's content id, so same-path byte replacement misses; `ExecutionLaneKey` isolates concrete device route + placement + backend. Targeted content-id eviction / pool clear replace generation-tagged lazy invalidation; `release_session_scoped_buffers` runs before the actor returns idle. |
...
| Zero-copy audio path (no avoidable resample/copy hops) | Supported | Fixed by PR #175 (`7c96ae0`). Direct 16kHz-mono-WAV input (`api/audio_io.rs:5` -> `models/kaldi_fbank.rs:115`) remains the 2 necessary full-buffer hops (disk read, byte->f32 parse); the fbank frame loop reuses per-frame buffers. The previously-avoidable double round trip is gone: non-WAV input now decodes via symphonia straight to an in-memory `Arc<Vec<f32>>` (`PreparedAudioSamples::InMemory`, `audio/types.rs:111-113`) and the temp-WAV write/re-read/re-parse is skipped entirely for that path (`audio/prepare.rs`); WAV passthrough and the external ffmpeg/afconvert fallbacks keep the path form (nothing to improve there). The common in-memory path is zero-copy end to end (`Arc::try_unwrap` at the single consumer); only the word-timestamps refine path takes one honest clone. Also a precision improvement: full f32 now reaches the frontend instead of a lossy f32->i16->f32 round trip through the temp WAV. All four request-construction sites wired (CLI transcribe/live/native-segment, bench suite); no committed golden changes (fixtures are all WAV). |
