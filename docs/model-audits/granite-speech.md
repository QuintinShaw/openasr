...
| Resident pool reuse across requests (weights stay resident) | Supported | K2 is derived from the Granite architecture descriptor's `module_slug = "granite_speech"`, `PreparedRuntimePool`, NativeExecutionServices ownership, and content-id eviction facets; `resident_runtime_audit` then scans the derived module for the shared owner-bound cache primitive. Prepared runtime (encoder + projector + decode session + embed table) is cached cross-request; `release_session_scoped_buffers` resets session-scoped state before re-store. |
